...
contract VictoriaVRLand is ERC721Enumerable, Ownable {
    ...
    event Whitelist(address user, uint8[] tiers);
    ...
    function whitelistUsers(address[] calldata users, uint8[][] calldata tiers) external onlyOwner {
        ...
        for (uint i = 0; i < users.length; i++) {
            ...
            emit Whitelist(users[i], tiers[i]);
        }
    }
